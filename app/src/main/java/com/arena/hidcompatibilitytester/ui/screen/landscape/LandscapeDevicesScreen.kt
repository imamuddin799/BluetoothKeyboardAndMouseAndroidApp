...
fun LandscapeDevicesScreen(
    ...
    LazyColumn(
        ...
        if (nearbyList.isEmpty()) {
            item {
                Text(
                    "Tap Scan to discover devices.", fontSize = 13.sp,
                    color = Color.Gray, modifier = Modifier.padding(start = 8.dp)
                )
            }
        } else {
            items(nearbyList) { device ->
                LandscapeDeviceRow(name = device.name ?: "Unknown", address = device.address) {
                    Button(
        ...
        item {
            Text(
                "Paired Devices", fontSize = 15.sp, fontWeight = FontWeight.SemiBold,
        ...
        if (pairedList.isEmpty()) {
            item {
                Text(
                    "No paired devices.", fontSize = 13.sp,
                    color = Color.Gray, modifier = Modifier.padding(start = 8.dp)
                )
            }
        } else {
...
@Composable
private fun LandscapeDeviceRow(
    name: String,
    address: String,
